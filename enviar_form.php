...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
   ...
   $sql = "INSERT INTO triagem_table (uf, localidade, circuito, produto, reclamacao, posto, data_abertura, data_vencimento, nome_cliente, observacao, emissor, status, detalhes,especialista_responsavel) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    // Prepare e vincule os parâmetros para evitar injeção de SQL
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssssssssss", $uf, $localidade, $circuito, $produto, $reclamacao, $posto, $data_abertura, $data_vencimento, $nome_cliente, $observacao, $emissor, $status, $detalhes, $especialista_responsavel);
    ...
    if ($stmt->execute()) {
        ...
        echo '<script>window.location.href = "formulario_triagem.php";</script>';
    } else {
